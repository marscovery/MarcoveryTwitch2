...
namespace Simple_Twitch
{
    public class DamageIndicator
    {
        ...
        private static void OnEndScene(EventArgs args)
        {
            ...
            if (Program.DrawEOnEnemies)
            {
                foreach (var hero in EntityManager.Heroes.Enemies
                    .Where(x => x.IsValidTarget()
                    && x.IsHPBarRendered
                    && x.HasBuff("TwitchDeadlyVenom")
                    && Program.ArgsE.IsInRange(x)))
                {
                    _height = 9;
                    _width = 104;
            ...
            if (Program.DrawEOnCamps)
            {
                foreach (
                var unit in
                EntityManager.MinionsAndMonsters.GetJungleMonsters(Player.Instance.Position, 1200)
                        .Where(x => x.IsValidTarget()
                                    && x.IsHPBarRendered
                                    && x.HasBuff("TwitchDeadlyVenom")))
                {
        ...
        private static void DrawLine(Obj_AI_Base unit)
        {
            ...
            //Draw the line
            Drawing.DrawLine(startPoint, yPos, endPoint, yPos, _height, unit is AIHeroClient ? Program.DrawingColors[2].CurrentValue : JungleColor);
        }
    }
}